...
const Header = ({ roundedHeaderButton = false, logoLink, links, className, collapseBreakpointClass = "lg" }) => {
  ...
  const defaultLogoLink = (
    <a href="/" className="flex items-center font-black border-b-0 text-2xl ml-0 text-blue-900">
      <img src="/path/to/default-logo.png" alt="logo" className="w-10 mr-3" />
      EPC
    </a>
  );
  ...
  return (
    <header className={`flex justify-between items-center max-w-screen-xl mx-auto ${className || "header-light"}`}>
      ...
      <nav className={`flex flex-1 items-center justify-between lg:hidden ${collapseBreakpointCss.mobileNavLinksContainer}`}>
        {logoLink}
        <motion.div initial={{ x: "150%", display: "none" }} animate={animation} className={`lg:hidden z-10 fixed top-0 inset-x-0 mx-4 my-6 p-8 border text-center rounded-lg text-blue-900 bg-white ${collapseBreakpointCss.mobileNavLinks}`}>
          {links}
        </motion.div>
        <button 
